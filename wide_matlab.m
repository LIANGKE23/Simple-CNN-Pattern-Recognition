...
%%
rng('default');
numEpochs = 15; % For aug 30 for original 15
batchSize = 75;
nTraining = length(train.Labels);

% here we add another set of "CONV -> ReLU -> POOL ->" to make the network:
% CONV -> bacthNorm -> ReLU -> POOL ->CONV -> bacthNorm -> ReLU -> POOL -> FC -> DROPOUT -> FC -> SOFTMAX 
layers = [
...
%     imageInputLayer([256 256 1]); % Input to the network is a 256x256x1 sized image 
    ...
    fullyConnectedLayer(25); % Fullly connected layer with 200 activations
%     batchNormalizationLayer()
%     reluLayer();
    dropoutLayer(.60); % Dropout layer
